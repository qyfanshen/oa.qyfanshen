...
const emptyForm: FormData = { type: "", documentName: "", copies: 1, purpose: "", urgency: "normal" };
...
export default function SealPage() {
  ...
  const [myRecords, setMyRecords] = useState<SealRecord[]>([]);
  const [allRecords, setAllRecords] = useState<SealRecord[]>([]);
  ...
  const loadRecords = async () => {
    const response = await fetch("/api/seal", { cache: "no-store" });
    if (!response.ok) return;
    const data = (await response.json()) as { records?: SealRecord[] };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.type || !form.documentName.trim() || !form.purpose.trim() || form.copies < 1) {
      setError("请完整填写有效的用章信息。");
      return;
    }
    ...
    const response = await fetch("/api/seal", {
      ...
      body: JSON.stringify({ ...form, documentName: form.documentName.trim(), purpose: form.purpose.trim() }),
    }).catch(() => null);
    setSubmitting(false);
    ...
    if (!response?.ok) {
      ...
      if (response?.status === 409 && data?.duplicate) {
        ...
        setForceForm({ ...form, documentName: form.documentName.trim(), purpose: form.purpose.trim() });
    ...
    setNotice("用章申请已提交，管理端审批中心会同步显示。");
  ...
  const confirmForceSubmit = async () => {
    ...
    setSubmitting(true);
    const response = await fetch("/api/seal?force=1", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(forceForm),
    }).catch(() => null);
    setSubmitting(false);
    setForceForm(null);
    if (!response?.ok) {
  ...
  const reviewSeal = async (id: string, status: "approved" | "rejected") => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-800">公章审批</h2>
          <p className="mt-1 text-sm text-gray-400">
            员工提交用章申请后会进入管理端审批中心，审批结果会同步回用章记录。
          </p>
        </div>
        <button
          type="button"
      ...
      {demoRole === "employee" && (
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          <h3 className="mb-4 text-base font-semibold text-gray-800">我的用章记录</h3>
          <SealTable records={myRecords} showApplicant={false} />
        </div>
      )}

      {demoRole === "admin" && (
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          <h3 className="mb-4 text-base font-semibold text-gray-800">全部用章记录</h3>
          <SealTable records={allRecords} showApplicant onReview={reviewSeal} />
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h3 className="text-base font-semibold text-gray-800">用章申请</h3>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <label className="block text-sm text-gray-600">
                  ...
                  <input
                    ...
                    max={999}
                    value={form.copies}
                    onChange={(event) => setForm({ ...form, copies: Math.max(1, Number(event.target.value) || 1) })}
                    className="mt-1.5 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm focus:border-[#1e3a5f] focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/20"
                  />
                </label>
                <label className="block text-sm text-gray-600">
...
function SealTable({
  ...
  const colSpan = showApplicant ? 7 : 7;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100 bg-gray-50">
            ...
            <th className="px-3 py-2.5 text-left font-medium text-gray-500">印章类型</th>
            <th className="hidden px-3 py-2.5 text-left font-medium text-gray-500 sm:table-cell">文件名称</th>
            <th className="px-3 py-2.5 text-center font-medium text-gray-500">份数</th>
            <th className="px-3 py-2.5 text-center font-medium text-gray-500">紧急</th>
          ...
          {records.map((record) => (
            <tr key={record.id} className="border-b border-gray-50 hover:bg-gray-50/50">
              ...
              <td className="px-3 py-2.5">
                <span className="rounded-full bg-amber-50 px-2 py-0.5 text-xs text-amber-600">{typeLabels[record.type] || record.type}</span>
              </td>
              <td className="hidden max-w-[150px] truncate px-3 py-2.5 text-xs text-gray-600 sm:table-cell">{record.documentName}</td>
              <td className="px-3 py-2.5 text-center text-gray-600">{record.copies}份</td>
              <td className="px-3 py-2.5 text-center">
                <span className={`rounded-full px-2 py-0.5 text-xs ${record.urgency === "urgent" ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-500"}`}>
                  {urgencyLabels[record.urgency] || "普通"}
                </span>
              </td>
              <td className="px-3 py-2.5 text-center">
